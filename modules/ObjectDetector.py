...
class ObjectDetector(VideoStreamTrack):
    def __init__(self, config, service):
        ...
        self.out = cv.VideoWriter('outpy.mp4', cv.VideoWriter_fourcc(*'MP4V'), 30, (1280, 720))
        self.stream = cv.VideoCapture(config["bass"]["source"])
    ...
    def recv(self):
        while True:
            ...
            classes, confidences, boxes = self.net.detect(frame, confThreshold=0.1, nmsThreshold=0.1)  # 0.4 0.4
            if len(boxes) > 0:
                if self.use_meta:
                ...
                for classId, confidence, box in zip(classes.flatten(), confidences.flatten(), boxes):
                    ...
                    cv.putText(frame, label, (left - 50, top - 29), cv.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0),
                               2)
                    # vertical
                    cv.line(frame, (640, int(top + height / 2)), (640, 720), (255, 0, 0), thickness=3)
                    # hypot
                    cv.line(frame, (640, 720), (int(left + width / 2), int(top + height / 2)), (0, 255, 0), thickness=3)
                    # horizontal
                    cv.line(frame, (640, int(top + height / 2)), (int(left + width / 2), int(top + height / 2)),
                            (0, 255, 0), thickness=3)
                    cv.circle(frame, (640, 720),
                              int(math.sqrt(
                                  pow((640 - int(left + width / 2)), 2) + pow((720 - int(top + height / 2)), 2))),
                              (0, 255, 0),
                              thickness=3)
